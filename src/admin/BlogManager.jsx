...
import { adminFetchBlog, adminCreateBlog, adminUpdateBlog, adminDeleteBlog } from "../api";
import ImageUploader from "./ImageUploader";

const emptyForm = { title: "", slug: "", excerpt: "", imageUrl: "", date: "", category: "" };
...
function BlogManager() {
  ...
  function openEdit(item) {
    setEditId(item._id);
    setForm({ title: item.title, slug: item.slug, excerpt: item.excerpt || "", imageUrl: item.imageUrl || "", date: item.date || "", category: item.category || "" });
    setShowForm(true);
  }
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    try {
      if (editId) { await adminUpdateBlog(editId, form); }
  ...
  async function handleDelete(id) {
    ...
    try { await adminDeleteBlog(id); load(); } catch (err) { alert(err.message); }
  }
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
        ...
        <button onClick={openCreate} className="px-4 py-2 bg-sky-600 text-white rounded-lg text-sm font-semibold hover:bg-sky-700 transition w-full sm:w-auto">+ New Post</button>
      </div>

      {loading && <div className="bg-blue-50 text-blue-700 rounded-xl px-5 py-3 text-sm mb-4">Loading posts...</div>}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-start justify-center pt-0 sm:pt-10 pb-0 sm:pb-10 overflow-y-auto" onClick={() => setShowForm(false)}>
          <div className="bg-white shadow-xl w-full max-w-2xl sm:mx-4 sm:rounded-2xl min-h-screen sm:min-h-0 p-4 sm:p-6" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-bold text-slate-900">{editId ? "Edit Post" : "New Post"}</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input name="date" value={form.date} onChange={handleChange} placeholder="May 15, 2026" className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:border-sky-500" />
                </div>
                <div>
              ...
              <div>
                ...
                <textarea name="excerpt" value={form.excerpt} onChange={handleChange} rows={3} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:border-sky-500" />
              </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm hidden sm:table">
              ...
              <tbody className="divide-y divide-gray-100">
                ...
                {items.length === 0 && <tr><td colSpan={4} className="px-5 py-8 text-center text-gray-400">No posts yet</td></tr>}
              </tbody>
            </table>
            <div className="sm:hidden divide-y divide-gray-100">
                ...
                <div key={item._id} className="p-4 space-y-2">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      ...
                      <p className="text-xs text-gray-500 mt-0.5">{item.category} &middot; {item.date}</p>
                    </div>
              ...
              {items.length === 0 && <p className="text-center text-gray-400 py-8">No posts yet</p>}
            </div>
          </div>
        </div>
      )}
